...
def main():
    ...
    #주어진 Descriptor들을 numpy object에 저장합니다.
    try:
    ...
    #각 image의 Descriptor들을 200개씩 랜덤으로 가져옵니다.
    total_Des=selectSpecimen(SIFT_Des,200)
    print(total_Des.shape)

    #가져온 200,000개의 Descriptor들의 clustering center를 8개 찾습니다.
    try:
    ...
    #VLAD 알고리즘을 활용하여 각 이미지의 descriptor들과 clustering center와의 차이를 해당 이미지의 Descriptor로 저장합니다.
    try:
        with open('./pdata/my_Des.pickle', 'rb') as f:
            my_Des=pickle.load(f)
    except:
        my_Des = make_my_Des(SIFT_Des, center_points, NUM_CLUSTER)
        with open('./pdata/my_Des.pickle', 'wb') as f:
            pickle.dump(my_Des, f, pickle.HIGHEST_PROTOCOL)
    #만든 Descripotr를 eval 디렉토리에 binary file로 저장합니다.
...
def make_my_Des(SIFT_Des,center_points,num=NUM_CLUSTER):
    ...
    for imgIdx in range(len(SIFT_Des)):
        ...
        for desIdx in range(len(SIFT_Des[imgIdx])):
            ...
            minIdx = -1
            for classes in range(num):
                nowDist = L1Dist(center_points[classes], SIFT_Des[imgIdx][desIdx])
                if (minDist > nowDist):
                    minDist = nowDist
                    minIdx = classes
            center_idx[minIdx]=np.append(center_idx[minIdx],desIdx)
